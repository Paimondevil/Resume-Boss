export const scoreResume = (keywords, resumeText) => {
  if (!keywords || !Array.isArray(keywords)) {
    return {
      overall: 0,
      tier1: { total: 0, matched: 0, missing: [] },
      tier2: { total: 0, matched: 0, missing: [] },
      tier3: { total: 0, matched: 0, missing: [] },
    };
  }
  ...
  const check = (word) => {
    ...
    // Synonym map for soft skills and common variations
    const SYNONYMS = {
      "problem-solving": ["problem-solving", "problem solving", "troubleshoot", "debugging"],
      "analytical thinking": ["analytical", "analysis", "algorithms", "problem-solving"],
      ...
      "willingness to learn": ["continuous", "learning", "improvement"],
      ...
      "it service management": ["sdlc", "agile", "devops"],
      "team-oriented": ["team", "collaboration", "cross-functional"],
      "full-stack": ["react", "frontend", "backend", "api"],
      "full stack": ["react", "frontend", "backend", "api"],
    };
    ...
    // Multi-word phrase — all meaningful parts must appear
    if (w.includes(" ")) {
  ...
  return {
    ...
    tier1: {
      total: tier1.length,
      matched: matchedT1.length,
      missing: tier1.filter(k => !check(k.word)).map(k => k.word),
    },
    tier2: {
      total: tier2.length,
      matched: matchedT2.length,
      missing: tier2.filter(k => !check(k.word)).map(k => k.word),
    },
    tier3: {
      total: tier3.length,
      matched: matchedT3.length,
      missing: tier3.filter(k => !check(k.word)).map(k => k.word),
    },
  };
};